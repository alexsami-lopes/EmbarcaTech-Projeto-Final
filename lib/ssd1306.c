...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  ...
  {
    index = (c - 'a' + 37) * 8; // Letras minúsculas em 37
  }else  if (c >= '0' && c <= '9')
  {
    index = (c - '0' + 1) * 8; // Números
  } else  if (c >= '[' && c <= '_')
  {
    index = (c - '[' + 63) * 8; // Caracteres para borda
  } else  if (c == '@')
  {
    index = (c - '@' + 68) * 8; // Caracteres para borda
  } else  if (c == '*')
  {
    index = (c - '*' + 69) * 8; // Caracteres para borda
  } else  if (c == '.')
  {
    index = (c - '.' + 70) * 8; // Caracteres para borda
  } else  if (c == ':')
  {
    index = (c - ':' + 71) * 8; // Caracteres para borda
  }
...
// Função para desenhar um caractere com pixels invertidos (efeito de seleção)
void ssd1306_draw_char_invert(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  uint16_t index = 0;
  ...
  {
    index = (c - '[' + 63) * 8; // Caracteres especiais
  } else  if (c == '@')
  {
    index = (c - '@' + 68) * 8; // Caracteres para borda
  } else  if (c == '*')
  ...
  for (uint8_t i = 0; i < 8; ++i)
  {
    ...
    for (uint8_t j = 0; j < 8; ++j)
    {
      // Inverte o estado do pixel: se o bit está setado, desenha 0; caso contrário, desenha 1.
      uint8_t pixel = (line & (1 << j)) ? 0 : 1;
      ssd1306_pixel(ssd, x + i, y + j, pixel);
    }
  }
}

// Função para desenhar uma string com cor invertida
void ssd1306_draw_inverted_string(ssd1306_t *ssd, const char *str, uint8_t x, uint8_t y)
{
  while (*str)
  {
    ssd1306_draw_char_invert(ssd, *str++, x, y);
    x += 8;
    if (x + 8 >= ssd->width)
    {
      x = 0;
...
void ssd1306_border_character(ssd1306_t *ssd, uint8_t top, uint8_t left, uint8_t width, uint8_t height, bool value, char character) {
    ...
    // Borda inferior:
    uint8_t bottom = top + height - CARD_HEIGHT;
    for (uint8_t x = left; x < left + width; x += CARD_WIDTH) {
        ssd1306_draw_char(ssd, character, x, bottom);
    }
    // Borda esquerda:
    for (uint8_t y = top; y < top + height; y += CARD_HEIGHT) {
        ssd1306_draw_char(ssd, character, left, y);
    }
    // Borda direita:
    uint8_t right = left + width - CARD_WIDTH;
    for (uint8_t y = top; y < top + height; y += CARD_HEIGHT) {
